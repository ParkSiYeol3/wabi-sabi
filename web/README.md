...
- **2026-07-26 버튼 로딩 피드백 전수(#269)**: `SubmitButton`(useFormStatus) — bare 서버액션 폼에 붙이면 제출 중 pendingText. 어드민·고객 피드백 없던 버튼 전수 적용 + 담기 "담겼습니다 ✓". product-gallery `preload`→`priority` 픽스. **상품 이미지 화질 저하는 업로드 원본 저해상도(360×540 등)가 원인 — 고해상도 재업로드 필요**(코드/설정 무관).
