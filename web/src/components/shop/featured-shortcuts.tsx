...
// 대표님: 오늘의 와비사비가 우리 사이트의 특색 → 대분류로 승격해 shop·showroom 등
// 여러 페이지 상단에 노출한다. 폭은 자연 폭(내용에 맞춤) — 모바일에서 전폭으로
// 늘리면(flex-1) 넓고 낮아 비정형 원이 얇아 보여, BackToShop 버튼처럼 통통한
// blob 이 되도록 넉넉한 패딩의 자연 폭 알약으로 통일(대표님). 드로어 SHOP 하위엔 없음.
// (월간 그릇은 오프라인 매장 전용으로 전환돼 제거 — 대표님.)
// 돌의 정원(#616)이 그 빈자리에 들어가 알약이 다시 둘이 된다 — SHAPES 가 서로
// 다른 두 값을 갖고 있어 나란히 놓여도 균질하지 않다.
const ITEMS = [
  { href: "/today", label: "오늘의 와비사비" },
  // 이름은 間(마) — 돌과 모래 사이의 여백(대표님). 주소는 /garden 그대로 둔다:
  // 이미 색인·사이트맵에 올라간 주소를 바꾸면 얻는 것 없이 링크만 끊긴다.
  { href: "/garden", label: "間, 마" },
] as const;
