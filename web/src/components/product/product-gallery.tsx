...
// 전체 폭에 불규칙하게 흩어 놓는다 — 폭·좌우 위치(margin-left)·간격을 층마다 달리해
// 열 맞춤 없이 흩뿌린다. 비율은 강제하지 않아 사진마다 원본 비율 그대로 표시된다
// (대표님 — 올린 모양대로). 스크롤을 내리면 히어로(사진+정보)는 위로 사라지고 이
// 사진들만 쭉 이어진다. 상태 없는 서버 컴포넌트 — 인덱스 기반 결정적(SSR 안정).
//
...
// 비율은 강제하지 않는다(대표님 — 원본 비율 그대로). 각 사진이 자기 높이를 만든다.
    ...
    <div className="mt-10 md:mt-16">
      {images.map((src, i) => {
        ...
        return (
          <div
            key={i}
            className={`relative bg-wabi-muted ${gap} ${SLOTS[i % SLOTS.length]}`}
          >
            <ProductImageZoom
              src={src}
              alt={`${name} 상세 이미지 ${i + 2}`}
