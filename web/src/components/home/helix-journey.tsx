...
const DESKTOP = {
  vb: "0 0 1000 2890",
  // 첫 멘트도 로드 시 바로 뜨지 않게(#348 후속) — 모바일과 동일하게 앞쪽 리드인
  // 코일을 둔다. loops 6→7, 멘트를 극점 k=[3,8,13]로 옮겨 侘 앞 2반바퀴가 먼저
  // 그려진다. 피치·반경·타원비·멘트 간격(5)·좌우교차는 그대로.
  geom: helixSegments(500, 300, 430, 0.3, 95, 2825, 7, 448),
  moments: momentPositions(300, 430, 95, 2825, 2890, 7, [3, 8, 13]),
  axis: [65, 2855] as const,
  fade: 0.32, // 멘트 페이드 구간(±fade·vh) — 종형 반폭
};
// 모바일: 첫 멘트를 "로드 후 시간 페이드인"(render i===0 분기)으로 띄우므로 리드인
// 코일이 필요 없다 → 첫 멘트를 상단(k2)에 둬 회전수를 줄인다(대표님 — 첫 멘트 숨김이
// 리드인=회전수를 늘리던 걸 제거). loops 6, 피치 560, rYRatio 0.42.
// 멘트 극점 4반바퀴 간격 k=[2,6,10] — 전부 우측 극점이라 텍스트가 전부 좌측 여백에
// 깨끗이 앉는다(점은 우측 끝, 텍스트는 좌측 → 안 겹침). 긴 본문 選(k10)이 교차 타원
// 렌즈에 들어가도록 텍스트 폭을 넓혀(아래 ci===1 className, ~65%) 5줄→4줄로 줄였다.
// 간격 32%(≈408px)라 멘트 페이드가 거의 겹치지 않는다.
const MOBILE = {
  vb: "0 0 1000 3480",
  geom: helixSegments(500, 320, 450, 0.42, 120, 3360, 6, 480),
  moments: momentPositions(320, 450, 120, 3360, 3480, 6, [2, 6, 10]),
  axis: [90, 3420] as const,
  // 모바일은 페이드 구간을 좁혀(0.16) 멘트 간 겹침을 막고(멘트 peak 간 ~280px >
  // 페이드 윈도우 270px), 첫 멘트가 리드인 코일 끝(뷰포트 아래)에서 확실히 0 으로
  // 시작하게 한다(로드 시 侘·寂·選 opacity 0 — 실측).
  fade: 0.16,
};

// 모바일 멘트 세로 미세보정(%): 중간 밀도(loops7)에선 세 멘트가 극점 반대쪽 여백에
// 앉아(dotLeft 분기) 보정이 필요 없다. 곡선 밀도/멘트 극점 배치 바뀌면 재조정.
const MOBILE_MOMENT_NUDGE = [0, 0, 0];
    ...
    const render = (top: number) => {
      ...
      CANVASES.forEach((cfg, ci) => {
        ...
        cfg.moments.forEach((pos, i) => {
          ...
          let vis: number;
          if (i === 0) {
            // 첫 멘트: 페이드인은 로드 후 시간으로(스크롤 없이 무→서서히 등장, 대표님),
            // 페이드아웃은 점이 화면 상단으로 올라가면 위치로. 리드인 코일 없이도 로드 시
            // opacity 0 이라, 첫 멘트 숨김이 회전수를 늘리지 않는다(리드인 제거).
            const tIn = clamp01((performance.now() - introT0) / 1600);
            const fadeIn = tIn * tIn * (3 - 2 * tIn); // smoothstep
            // 페이드아웃: 점이 화면 상단(≈6%)으로 올라가 이탈할 때 사라진다. 로드 시
            // 점이 상단부(중앙 위)에 있어도 보이고, 스크롤로 상단 밖으로 나갈 때만 소멸.
            const fadeOut = clamp01((dotY - vh * 0.06) / (vh * cfg.fade));
            vis = Math.min(fadeIn, fadeOut);
          } else {
            vis = clamp01(1 - Math.abs(dotY - vh * 0.5) / (vh * cfg.fade));
          }
              ...
              return (
                <div key={v.han}>
                  ...
                  <div
                    ...
                    className={
                      ci === 1
                        ? // 모바일: 멘트를 극점 반대쪽 여백에 앉힌다 — 점은 극점(좌/우 끝)에,
