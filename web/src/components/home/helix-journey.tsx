...
// 멘트 = 나선 8바퀴의 극점 k=1(좌)·8(우)·15(좌) — 3주(#225)라 간격을
// 7반바퀴로 넓혔다(홀수 간격 = 좌우 교차 유지). 리드인을 반바퀴로 줄여
// 곡선이 첫 글씨(侘) 직전에 시작한다(대표님 피드백 — 곡선 길이 단축).
// 상하 여백을 같게(yStart = vbH−yEnd) 두어 3주가 캔버스 중앙에 대칭 배치 —
// 2번째(寂)가 정확히 50%, 1·3번째가 그 위아래 등간격(대표님 피드백 — 간격 일정).
// 테이퍼라 극점 x 가 층마다 다르다: x = 50 ∓ R(k)/10, R(k)=300+8.125k.
// y 는 여백비를 두 캔버스가 공유하므로 동일:
// y% = (yStart + (yEnd−yStart)·k/16) / vbH.
const MOMENT_POS = [
  { x: 19.2, y: 8.8 },
  { x: 86.5, y: 50.0 },
  { x: 7.8, y: 91.2 },
...
// 8바퀴/캔버스 — 리드인(첫 글씨 위 빈 코일)을 반바퀴로 줄여 곡선이 侘 직전에
// 시작한다. 상하 여백을 같게(yStart = vbH−yEnd) 두어 3주가 캔버스 중앙 대칭 —
// 2번째(寂) 50%, 1·3번째가 등간격(대표님 피드백 — 간격 일정하게).
const DESKTOP = { vb: "0 0 1000 3310", geom: helixSegments(500, 300, 430, 0.3, 95, 3215, 8, 512) };
const MOBILE = { vb: "0 0 1000 6860", geom: helixSegments(500, 300, 430, 0.3, 197, 6663, 8, 512) };
