...
//
// 모바일: 세로로 길고(피치↑ 멘트 간격 확보) 코일을 좁힌다(175→230) — 폭을 꽉
// 채우면 납작해 보이고 좌우에 멘트 포켓이 안 나온다(선을 뚫던 원인). 다만 캔버스가
// 2배 길고 반지름이 좁아 진폭이 작으면 고리가 안 겹쳐 "납작한 1차원 S"로 보인다.
// 3D 코일(고리 교차)은 타원 진폭 ≈ 반바퀴 y이동일 때 생긴다 → 좁은 반지름은
// 유지하되 진폭(타원비)을 키워 겹침을 복원한다(데스크톱과 같은 입체감).
// loops 는 코일 밀도 — 많으면 촘촘한(압축) 스프링, 적으면 성긴(열린) 스프링.
// 좁은 폭에서 성기게 열려면 진폭을 더 키워야(타원비↑) 겹침이 유지된다. 7 loops +
// 타원비 0.85 로 압축감을 줄였다(대표님/시열님 — 너무 촘촘). steps 는 64/loop.
// 극점 좌표(moments)는 각 반지름에서 계산해 점·멘트가 곡선에 정확히 맞는다.
const DESKTOP = {
  vb: "0 0 1000 2530",
...
const MOBILE = {
  vb: "0 0 1000 5244",
  geom: helixSegments(500, 175, 230, 0.85, 197, 5047, 7, 448),
  moments: momentPositions(175, 230, 197, 5047, 5244, 7),
