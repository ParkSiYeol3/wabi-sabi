...
// 모바일: 곡선을 웹처럼 납작한 타원으로(대표님 — 둥근 큰 원 ✗). rYRatio 0.42→0.33
// (웹 0.3 근접)으로 납작·넓은 교차 렌즈. 회전수는 6~7 이 예쁘고 8 은 과하다(대표님)
// → loops 7. 첫 멘트는 모바일 전용 시간 페이드(render i===0&&ci===1)라 리드인 코일이
// 필요 없어 상단(k2)에 두고 회전수를 억제한다. 멘트 극점 k=[2,6,10] 전부 우측 극점 →
// 텍스트 전부 좌측 여백(납작 렌즈는 폭이 넓어 극점 반대쪽만 안전). 텍스트·본문 크기는
// 배포본(40/52·14/17)으로 유지(대표님 — 그게 제일 보기 좋음).
const MOBILE = {
  vb: "0 0 1000 3880",
  geom: helixSegments(500, 320, 450, 0.33, 120, 3760, 7, 560),
  moments: momentPositions(320, 450, 120, 3760, 3880, 7, [2, 6, 10]),
  axis: [90, 3820] as const,
  // 페이드 구간(±0.16·vh). 멘트 peak 간격이 넉넉해(≈27% 캔버스) 겹치지 않는다.
  fade: 0.16,
};

// 모바일 멘트 세로 미세보정(%): 우측 극점 배치라 세 멘트가 좌측 여백에 자연히 앉아
// 보정이 필요 없다. 곡선 밀도/멘트 극점 배치 바뀌면 재조정.
    ...
    const render = (top: number) => {
      ...
      CANVASES.forEach((cfg, ci) => {
        ...
        cfg.moments.forEach((pos, i) => {
          ...
          let vis: number;
          if (i === 0 && ci === 1) {
            // 첫 멘트(모바일만): 시간 페이드인(로드 시 0→서서히 등장), 페이드아웃은 점이
