...
// 모바일: 웹과 동일한 곡선 형태(대표님) — 웹은 "납작·넓은 타원이 촘촘히 쌓인
// 스프링"인데 예전 모바일은 루프가 세로로 늘어나(피치 780/루프) 종횡비 0.9 의 큰
// 스윙이었다(웹 0.48). 원인은 rYRatio 가 아니라 피치 — rYRatio 만 낮추면 타원이
// 서로 멀어져 가는 슬라이버가 됐다(#352 반려). 두 레버를 함께 잡되(rYRatio 0.3=웹
// 납작 타원, 피치는 웹급으로 낮게), 좁은 세로화면에서 회전수가 과해 보이지 않게
// loops 는 8 로 억제한다(대표님 — 11 은 과함). 피치 460/루프(종횡비 ~0.6)로 웹의
// 납작함을 유지하면서 화면당 루프 밀도를 낮췄다. 멘트는 극점 4반바퀴 간격
// (k=[6,10,14], 모두 우측 극점)으로 좌측 여백에 텍스트가 깨끗이 앉고, 첫 멘트 侘 를
// 캔버스 39%(리드인 3바퀴)에 둬 로드 시 화면중앙 밖에서 시작 → 페이드인. 뒤 1바퀴 꼬리.
const MOBILE = {
  vb: "0 0 1000 3800",
  geom: helixSegments(500, 320, 450, 0.3, 120, 3680, 8, 640),
  moments: momentPositions(320, 450, 120, 3680, 3800, 8, [6, 10, 14]),
  axis: [90, 3740] as const,
  // 모바일은 페이드 구간을 좁혀(0.16) 멘트 간 겹침을 막고, 첫 멘트가 리드인 코일
  // 끝(뷰포트 아래)에서 확실히 0 으로 시작하게 한다(로드 시 侘 opacity 0 — 실측).
  // loops 11→8 로 줄여 회전수가 과해 보이던 걸 완화(대표님) — 피치를 400→460 으로
  // 살짝 키워 화면당 루프 밀도도 낮췄다. rYRatio 0.3(웹 납작 타원)은 유지. loops 8 이
  // 회전수↓·첫멘트 숨김(리드인 3바퀴 확보)·멘트 간격을 동시에 만족하는 하한.
  fade: 0.16,
};
...
const MOBILE_MOMENT_NUDGE = [0, 0, -13];
