...
// 서로 멀어져 가는 슬라이버가 됐다(#352 반려). 이번엔 두 레버를 함께: loops 6→9 로
// 피치를 780→400(웹 390 수준)으로 낮추고 rYRatio 0.5→0.3(웹과 동일)으로 납작하게.
// 피치가 낮아 타원이 가까워지니 슬라이버가 아니라 웹처럼 촘촘한 납작 스프링이 된다.
// 멘트는 극점 6반바퀴 간격(k=[4,10,16])으로 벌려 겹침을 막고, 모두 우측 극점이라
// 좌측 여백에 텍스트가 깨끗이 앉는다. 앞 2바퀴는 리드인, 뒤 1바퀴는 로고 꼬리.
const MOBILE = {
  vb: "0 0 1000 4620",
  geom: helixSegments(500, 320, 450, 0.3, 120, 4520, 11, 880),
  moments: momentPositions(320, 450, 120, 4520, 4620, 11, [8, 14, 20]),
  axis: [90, 4560] as const,
...
const MOBILE_MOMENT_NUDGE = [0, 0, -10];
