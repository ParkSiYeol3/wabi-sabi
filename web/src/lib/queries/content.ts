...
// (lib/shipping: 10만원 이상 무료·미만 3,500원)로 서버가 계산한다. 기본 문구는 그
...
뜨거운 음식을 담기 전, 미지근한 물로 기물을 먼저 데우면 온도 차로 인한 균열을 줄일 수 있습니다.
무광 기물은 커트러리와의 마찰로 자연스러운 흔적이 남을 수 있어, 우드 커트러리를 함께 쓰시길 권합니다.`;
export const CARE_MAINTAIN_KEY = "care_maintain";
export const DEFAULT_CARE_MAINTAIN = `음식에 따라 표면에 옅은 색이 배거나 일시적으로 변색될 수 있습니다.
